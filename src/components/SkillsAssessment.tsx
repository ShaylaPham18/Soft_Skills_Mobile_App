...
export function SkillsAssessment({ user, accessToken }: SkillsAssessmentProps) {
  ...
  const completeAssessment = async () => {
    ...
    const calculatedResults = Object.entries(skillScores).map(([skill, data]) => ({
      ...
      level: data.total / data.count >= 4 ? 'Strong' : data.total / data.count >= 3 ? 'Average' : 'Needs Improvement'
    }));
    ...
    try {
      ...
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-93cd01be/save-assessment`, {
        method: 'POST',
        headers: {
              ...
              {results.map((result: any, index: number) => (
                <div key={index} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={result.level === 'Strong' ? 'default' : result.level === 'Average' ? 'secondary' : 'destructive'}>
                      {result.level}
                    </Badge>
                  </div>
          ...
          <Card className="border-blue-200 dark:border-blue-800">
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground mt-3">
                Focus on challenges that target these areas for maximum improvement.
              </p>
            </CardContent>
          </Card>
        )}
  ...
  return (
    <Card className="border-blue-200 dark:border-blue-800">
      ...
      <CardContent className="space-y-6">
        <div>
          <Badge className="mb-3 bg-blue-100 text-blue-800 hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-200">{currentQ.skill}</Badge>
          <h3 className="font-medium mb-4 text-blue-900 dark:text-blue-100">{currentQ.question}</h3>
